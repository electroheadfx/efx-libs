		...
		stats: [
			{
				...
				value: Math.round(rng() * 5000).toLocaleString(),
				change: `+${(rng() * 10).toFixed(0)}%`,
			},
			{
				label: "Bounce Rate",
				value: `${(20 + rng() * 30).toFixed(1)}%`,
				change: `-${(rng() * 5).toFixed(0)}%`,
			},
			{
				label: "Avg. Session",
				value: `${Math.floor(rng() * 10)}m ${Math.floor(rng() * 60)}s`,
				change: `+${Math.floor(rng() * 30)}s`,
			},
			{
				label: "Page Views",
				value: `${(rng() * 100).toFixed(1)}K`,
				change: `+${(rng() * 15).toFixed(0)}%`,
			},
		],
	};
}

...
import type { LayoutTemplate, LayoutPadding } from './DashboardLayout';
...
export function ResponsiveDashboardLayout({
  ...
  const template = breakpoint === 'mobile' 
    ? templates.mobile 
    : breakpoint === 'tablet' 
  ...
  const getOffset = (): string => {
    ...
    if (typeof viewportOffset === 'string' || typeof viewportOffset === 'number') {
      return typeof viewportOffset === 'number' ? `${viewportOffset}px` : viewportOffset;
    }

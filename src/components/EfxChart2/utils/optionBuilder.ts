...
/**
 * Loading opacity for dimmed sections (streaming mode)
 */
const LOADING_OPACITY = 0.15
...
export function buildEChartsOption(
  ...
  for (const section of sections) {
    ...
    // Check if section is loading (streaming mode)
    const isLoading = sectionLoadingStates?.[section.section] ?? false
    const opacity = isLoading ? LOADING_OPACITY : 1
    ...
    if (sectionTitle) {
      ...
      titles.push({
        ...
        left: "center",
        top: titleTop,
        // Only apply loading opacity when loading, otherwise use default
        textStyle: isLoading
          ? { ...sectionTitle.textStyle, opacity }
    ...
    if (section.type) {
      ...
      // Only apply loading opacity when actually loading
      // When loaded, let ECharts use its natural default opacity values
      if (isLoading) {
        // Apply opacity only to elements that exist, preserving their show state
        const xAxisRecord = xAxis as Record<string, unknown>
        const yAxisRecord = yAxis as Record<string, unknown>

        // Helper to apply loading opacity to an axis element if it exists
        const applyLoadingOpacity = (axis: Record<string, unknown>, prop: string) => {
          const existing = axis[prop] as Record<string, unknown> | undefined
          if (existing) {
            axis[prop] = {
              ...existing,
              lineStyle: { ...(existing.lineStyle as object), opacity },
            }
          }
        }

        // Apply to xAxis elements that already exist
        applyLoadingOpacity(xAxisRecord, 'axisLine')
        applyLoadingOpacity(xAxisRecord, 'axisTick')
        applyLoadingOpacity(xAxisRecord, 'splitLine')
        if (xAxisRecord.axisLabel) {
          xAxisRecord.axisLabel = { ...(xAxisRecord.axisLabel as object), opacity }
        }

        // Apply to yAxis elements that already exist
        applyLoadingOpacity(yAxisRecord, 'axisLine')
        applyLoadingOpacity(yAxisRecord, 'axisTick')
        applyLoadingOpacity(yAxisRecord, 'splitLine')
        if (yAxisRecord.axisLabel) {
          yAxisRecord.axisLabel = { ...(yAxisRecord.axisLabel as object), opacity }
        }
      }
      ...
      // Only apply loading opacity to series when actually loading
      if (isLoading) {
        const seriesRecord = seriesConfig as Record<string, unknown>
        if (seriesRecord.lineStyle) {
          seriesRecord.lineStyle = { ...(seriesRecord.lineStyle as object), opacity }
        } else {
          seriesRecord.lineStyle = { opacity }
        }
        if (seriesRecord.itemStyle) {
          seriesRecord.itemStyle = { ...(seriesRecord.itemStyle as object), opacity }
        } else {
          seriesRecord.itemStyle = { opacity }
        }
        if (seriesRecord.areaStyle) {
          seriesRecord.areaStyle = { ...(seriesRecord.areaStyle as object), opacity: opacity * 0.5 }
        }
      }
